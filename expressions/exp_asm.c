...
BOOL FASTCALL GetOperands(int opid)
{
	...
	if((opcode->paramTypes&prmNONE) &&
	   comProcSub_OpParamNONE(opcode->codes[priNONE], &brackCnt) )
	{
		//
	}           
	else
	...
	if((opcode->paramTypes&prmA) &&
	   comProcSub_OpParamA(opcode->codes[priA], &brackCnt) )
	{
	}
	else
	// catch it if it has a relative address as it's operand
	if((opcode->paramTypes&prmREL) &&
	   comProcSub_OpParamREL(opcode->codes[priREL], &brackCnt) )
	{
