        ...
        if (!(var->flags & VARFLAG_DEFCASTED) && (cast == VARCAST_STRUCT)) {
            ...
            if (PeekNextWord()[0] == '=') {
                ...
                if (var->flags & VARFLAG_TYPECAST) {
                    ...
                    elementsSize = 0;
                    elementsEnd = (GetBankOffset() - curBank->org) + ((arraySize == -1) ? var->size : var->size * arraySize);
                    ...
                    if (GetNextWord()[0] == '{') {
                        ...
                        braceCnt = 0;
                    do {
                        braceCnt = CompileVarBody(flags, brackCnt, var, &elementsSize, braceCnt, cast);
                    } while (braceCnt);
...
int CompileVarBody(U16 flags, S16* brackCnt, VAR* var, int* _elementsSize, int braceCnt, int cast)
{
    ...
    if (comProc_LabelDeclaration(flags, brackCnt)) {
        ...
        if (var->cast == VARCAST_STRUCT) {
            ...
            while (childvar) {
                braceCnt = CompileVarBody(flags | CF_VARCHILD, brackCnt, childvar, &elementsSize, braceCnt, childvar->cast);

                childvar = childvar->next;
                if (childvar && !braceCnt)
                    return 0;
            }
